...
export const WORK_CONFIG = {
    ...
    probabilityBySeason: {
        spring: 0.03,   // 春天万物复苏，打工概率很低
        summer: 0.05,   // 夏天打工概率较低
        autumn: 0.08,   // 秋天准备过冬，打工概率中等
        winter: 0.10    // 冬天需要更多资源，打工概率较高
    },
    // 打工时长（天）- 5-15天
    duration: {
        min: 5,
    ...
    // 打工薪资范围（每日薪资）- 【修改】大幅增加薪资
    salaryPerDay: {
        cat: { min: 50, max: 100 },       // 散户猫打工
        dog: { min: 60, max: 120 },       // 中产狗稳定收入
        bear: { min: 80, max: 160 },      // 大户熊
        fox: { min: 70, max: 140 },       // 游资狐
        tiger: { min: 120, max: 240 },    // 庄家虎赚大钱
        rabbit: { min: 55, max: 110 },    // 量化兔效率高
        cow: { min: 40, max: 80 },        // 神秘牛神秘收入
        user: { min: 80, max: 160 }       // 用户打工薪资大幅提高
    },
    // 哪些动物更可能打工（不交易时）- 【降低概率减少连续打工】
    workLikelihood: {
        cat: 0.08,       // 散户猫偶尔打工
        dog: 0.08,       // 中产狗偶尔打工
        bear: 0.02,      // 大户熊很少打工
        fox: 0.05,       // 游资狐有机会就打工
        tiger: 0.005,    // 庄家虎几乎不打工
        rabbit: 0.10,    // 量化兔偶尔打工优化资金
        cow: 0.005       // 神秘牛几乎不打工
    },
    // 【新增】连续打工冷却时间（天）- 防止连续打工
    workCooldownDays: 7
};
...
// 涨跌停情绪反应配置
export const LIMIT_REACTION_CONFIG = {
    // 涨停时的情绪倍数
    limitUpEmotionMultiplier: 1.5,
    // 跌停时的情绪倍数
    limitDownEmotionMultiplier: 1.5,
    
    // 各动物对涨停的反应
    limitUpReaction: {
        cat: { buyProbBoost: 0.3, sellProbBoost: -0.2 },      // 散户猫: 容易追涨
        dog: { buyProbBoost: 0.1, sellProbBoost: 0.0 },       // 中产狗: 理性观察
        bear: { buyProbBoost: 0.0, sellProbBoost: 0.3 },      // 大户熊: 趁高位卖出
        fox: { buyProbBoost: 0.4, sellProbBoost: -0.3 },      // 游资狐: 激进追涨
        tiger: { buyProbBoost: 0.2, sellProbBoost: 0.1 },     // 庄家虎: 控盘操作
        rabbit: { buyProbBoost: 0.2, sellProbBoost: -0.1 },   // 量化兔: 算法决策
        cow: { buyProbBoost: 0.0, sellProbBoost: 0.2 },       // 神秘牛: 趁高卖出
        panda: { buyProbBoost: 0.0, sellProbBoost: 0.1 },     // 懒熊猫: 略微卖出
        lion: { buyProbBoost: 0.3, sellProbBoost: 0.0 },      // 狮子王: 领涨
        elephant: { buyProbBoost: 0.0, sellProbBoost: 0.2 },  // 大象象: 稳健卖出
        wolf: { buyProbBoost: 0.3, sellProbBoost: -0.2 },     // 独狼哥: 追涨
        monkey: { buyProbBoost: 0.25, sellProbBoost: -0.15 }, // 调皮猴: 跟风追涨
        owl: { buyProbBoost: 0.1, sellProbBoost: 0.1 },       // 猫头鹰: 观察
        snake: { buyProbBoost: 0.35, sellProbBoost: -0.25 },  // 青蛇妹: 激进追涨
        pig: { buyProbBoost: 0.0, sellProbBoost: 0.25 }       // 存钱猪: 趁高卖出
    },
    
    // 各动物对跌停的反应
    limitDownReaction: {
        cat: { buyProbBoost: -0.2, sellProbBoost: 0.35 },     // 散户猫: 恐慌抛售
        dog: { buyProbBoost: 0.1, sellProbBoost: 0.1 },       // 中产狗: 观望
        bear: { buyProbBoost: 0.3, sellProbBoost: -0.2 },     // 大户熊: 抄底
        fox: { buyProbBoost: 0.2, sellProbBoost: 0.0 },       // 游资狐: 等机会
        tiger: { buyProbBoost: 0.25, sellProbBoost: -0.3 },   // 庄家虎: 抄底吸筹
        rabbit: { buyProbBoost: 0.15, sellProbBoost: 0.0 },   // 量化兔: 算法决策
        cow: { buyProbBoost: 0.2, sellProbBoost: -0.2 },      // 神秘牛: 神秘抄底
        panda: { buyProbBoost: 0.1, sellProbBoost: 0.0 },     // 懒熊猫: 躺平
        lion: { buyProbBoost: 0.2, sellProbBoost: -0.1 },     // 狮子王: 稳定市场
        elephant: { buyProbBoost: 0.25, sellProbBoost: -0.2 },// 大象象: 抄底
        wolf: { buyProbBoost: 0.2, sellProbBoost: -0.1 },     // 独狼哥: 独自抄底
        monkey: { buyProbBoost: -0.1, sellProbBoost: 0.2 },   // 调皮猴: 跟风恐慌
        owl: { buyProbBoost: 0.2, sellProbBoost: -0.1 },      // 猫头鹰: 理性抄底
        snake: { buyProbBoost: 0.1, sellProbBoost: 0.1 },     // 青蛇妹: 观察
        pig: { buyProbBoost: 0.15, sellProbBoost: -0.1 }      // 存钱猪: 小幅抄底
    }
};
...
export const generateOrderPrice = (animalKey, currentPrice, orderType, priceHistory, highestBuyPrice, monthStartPrice) => {
    ...
    if (orderType === 'sell' && highestBuyPrice && highestBuyPrice > 0) {
        ...
        orderLogger.debug(`📌 ${strategy.name} 卖单参考最高买单¥${highestBuyPrice.toFixed(3)}`);
    ...
    if (clampedPrice !== finalPrice) {
        orderLogger.debug(`📊 ${strategy.name} 订单价格被涨跌停限制: ¥${finalPrice.toFixed(3)} -> ¥${clampedPrice.toFixed(3)}`);
    }
...
// 根据策略决定买卖方向
// 【修改】增加 monthStartPrice 参数用于判断涨跌停
export const decideTradeDirection = (animalKey, player, currentPrice, priceHistory, monthStartPrice) => {
    const strategy = ANIMAL_STRATEGIES[animalKey];
    ...
    const totalMoney = (player.money || 0) + frozenMoney;
    const initialMoney = player.initialMoney || 1;

    // 【新增】资金不足时更倾向于卖出
    if (totalMoney < initialMoney * 0.2) {
        // 资金低于20%时，大幅增加卖出概率
        buyProbability -= 0.4;
        orderLogger.debug(`💰 ${strategy.name} 资金严重不足(${(totalMoney / initialMoney * 100).toFixed(0)}%)，倾向卖出`);
    } else if (totalMoney < initialMoney * 0.4) {
        // 资金低于40%时，增加卖出概率
        buyProbability -= 0.25;
        orderLogger.debug(`💰 ${strategy.name} 资金不足(${(totalMoney / initialMoney * 100).toFixed(0)}%)，倾向卖出`);
    } else if (totalMoney < initialMoney * 0.6) {
        // 资金低于60%时，轻微增加卖出概率
        buyProbability -= 0.1;
    }

    // 根据特殊行为调整
    if (behaviors.panicSell && totalMoney < initialMoney * 0.5) {
        // 恐慌抛售：亏损严重时增加卖出概率
        buyProbability -= 0.3;
    }
    ...
    if (monthStartPrice) {
        ...
        const limitReaction = LIMIT_REACTION_CONFIG.limitUpReaction[animalKey] || { buyProbBoost: 0, sellProbBoost: 0 };
        const limitDownReaction = LIMIT_REACTION_CONFIG.limitDownReaction[animalKey] || { buyProbBoost: 0, sellProbBoost: 0 };
        
        // 判断是否接近涨跌停（使用阈值而非精确匹配，因为价格可能有微小波动）
        const isNearLimitUp = currentPrice >= limitUpPrice * 0.99;
        const isNearLimitDown = currentPrice <= limitDownPrice * 1.01;
        
        if (isNearLimitUp) {
            // 接近涨停，根据动物特性调整
            buyProbability += limitReaction.buyProbBoost;
            buyProbability -= limitReaction.sellProbBoost; // sellProbBoost 是卖出概率增加，对买入是反向
            orderLogger.debug(`🔥 ${strategy.name} 接近涨停，情绪反应: 买入概率 ${(buyProbability * 100).toFixed(0)}%`);
        } else if (isNearLimitDown) {
            // 接近跌停，根据动物特性调整
            buyProbability += limitDownReaction.buyProbBoost;
            buyProbability -= limitDownReaction.sellProbBoost;
            orderLogger.debug(`❄️ ${strategy.name} 接近跌停，情绪反应: 买入概率 ${(buyProbability * 100).toFixed(0)}%`);
        }
    }
...
export const adjustForLiquidity = (animalKey, orderType, orderPrice, currentPrice, marketState, monthStartPrice) => {
    ...
    if (orderType === 'buy' && noSellDays >= liquidityConfig.noSellThreshold) {
        ...
        orderLogger.debug(`📊 ${strategy?.name || animalKey} 因卖单不足 ${noSellDays} 天，提价 ${(priceBoost * boostMultiplier * 100).toFixed(1)}%`);
    }
    ...
    if (orderType === 'sell' && noBuyDays >= liquidityConfig.noBuyThreshold) {
        ...
        if (Math.random() < sellBoost) {
            ...
            orderLogger.debug(`📊 ${strategy?.name || animalKey} 因买单不足 ${noBuyDays} 天，倾向抛售`);
        }
    }
...
// 检查动物是否应该出门打工
export const checkShouldGoToWork = (animalKey, season, player, lastWorkEndDate) => {
    ...
    // 【新增】检查打工冷却期 - 防止连续打工
    if (lastWorkEndDate) {
        const cooldownDays = workConfig.workCooldownDays || 7;
        const daysSinceLastWork = Math.floor((Date.now() - new Date(lastWorkEndDate).getTime()) / (1000 * 60 * 60 * 24));
        if (daysSinceLastWork < cooldownDays) {
            orderLogger.debug(`⏳ ${strategy?.name || animalKey} 打工冷却中，还需等待 ${cooldownDays - daysSinceLastWork} 天`);
            return false;
        }
    }

    // 【增强】如果有股票（包括冻结股份），优先卖股票而不是打工
    const totalShares = (player.shares || 0) + (player.frozenShares || 0);
    if (totalShares > 0) {
        orderLogger.debug(`💰 ${strategy?.name || animalKey} 有 ${totalShares} 股，优先卖出而不是打工`);
        return false;
    }

    // 基础打工概率
    const seasonProbability = workConfig.probabilityBySeason[season] || 0.10;
    const animalLikelihood = workConfig.workLikelihood[animalKey] || 0.15;

    // 最终概率 = 季节概率 × 动物倾向
    const finalProbability = seasonProbability * animalLikelihood;

    // 资金不足时更可能打工 - 只有无股票时才打工
    let adjustedProbability = finalProbability;
    if (player.money <= 0) {
        // 完全没钱也没股票，必须打工
        orderLogger.log(`💼 ${strategy?.name || animalKey} 资金耗尽且无股票，强制外出打工！`);
        return true;
    } else if (player.money < player.initialMoney * 0.1) {
        // 资金低于10%且无股票，打工概率大幅增加
        adjustedProbability = Math.min(0.6, adjustedProbability * 5);
        orderLogger.log(`💼 ${strategy?.name || animalKey} 资金严重不足且无股票，急需打工！`);
    } else if (player.money < player.initialMoney * 0.2) {
        // 资金低于20%且无股票，打工概率翻倍
        adjustedProbability = Math.min(0.4, adjustedProbability * 3);
        orderLogger.log(`💼 ${strategy?.name || animalKey} 资金不足且无股票，需要打工`);
    }
...
export const getWorkIncome = (animalKey) => {
    const workConfig = WORK_CONFIG;
    const salaryPerDay = workConfig.salaryPerDay[animalKey] || workConfig.salaryPerDay.user;

    // 打工时长：5-15天
    const duration = workConfig.duration.min + Math.floor(Math.random() * (workConfig.duration.max - workConfig.duration.min + 1));

    // 每日薪资随机
    const dailySalary = salaryPerDay.min + Math.floor(Math.random() * (salaryPerDay.max - salaryPerDay.min + 1));

    // 总收入 = 每日薪资 × 天数（加一些随机加成）
    const bonus = Math.floor(dailySalary * duration * Math.random() * 0.3); // 0-30%加成
    const income = dailySalary * duration + bonus;

    return {
        income: Math.max(100, income),
        duration, // 打工时长（天）
...
export const getWorkMessage = (animalKey, isLeaving, income = 0) => {
    ...
    if (isLeaving) {
        const leavingMessages = {
            ...
            rabbit: `蹦蹦~ 执行外部套利任务中，预计5-15天后回归~`,
            cow: `哞~ ............出门.........`
        };
